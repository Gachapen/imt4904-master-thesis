%% Use short name MACS, MIS, CIMET, MTDMT, MIXD or MIS
%% Language english or norsk
%% b5paper with oneside or twoside, you can set A4 if you want but you submit in b5

%% If you want print with the heading material on a4 paper you can use this format
%% \documentclass[MACS,english,a4paper,oneside,12pt]{ntnuthesis/ntnuthesis}

%% with the change to using DAIM we have a new option.

\documentclass[MACS,english,DAIM]{ntnuthesis/ntnuthesis}

\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage{newunicodechar}
\usepackage{textcomp}
\usepackage{graphicx}
\usepackage{hyperref}

\usepackage{color}
\hypersetup{
    colorlinks=true,
    linkcolor=blue,
    citecolor=blue,
    filecolor=blue,
    urlcolor=blue
}

\usepackage{csvsimple}
\usepackage{url}
\usepackage[numbers]{natbib}
\usepackage{algorithm}
\usepackage{algpseudocode}
\usepackage{amsmath}
\usepackage{array}
\usepackage{tabularx}
\usepackage{makecell}
\setlength\extrarowheight{2pt}

\begin{document}

\input{inc/DaimData}

\makefrontpages

\input{inc/MastersIntro}

\tableofcontents

\hypersetup{pageanchor=true}

\listoffigures
\listoftables
\lstlistoflistings

\include{inc/01-Introduction}
\include{inc/02-Representation}
\include{inc/03-Combining}
\include{inc/04-Evaluation}
\include{inc/05-Discussion}
\include{inc/06-Related}
\include{inc/07-Conclusion}

\bibliographystyle{ntnuthesis/ntnuthesis}

\bibliography{Masters}

\end{document}
