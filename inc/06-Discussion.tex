...
As evident from the results, the \gls{DGEL} \gls{GE} process has the upper hand on
% TODO: perhaps say: performs significantly better than 
the brute-force approach, and the statistically significant difference in their scores medians makes a strong support for % TODO repeat the hypothesis here, so that the reader does not need to search for it back in text 
(\textbf{H1}).
...
Thus, the \gls{GE} process can save time by not evaluating certain individuals.
% TODO: What is the average percentage of evaluated individuals? Does this correlate with the difference in duration?
% TODO: yes - good questions. Also, it is a bit unclear why tournament doesn't need evaluation of all chromosomes - can you explain?
