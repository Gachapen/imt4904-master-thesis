...
Seen in Figure~\ref{fig:sa-progress-close} the points cluster around the same score, indicating that the locality of the grammar distribution mutations is good.
At the same time there are some points spread fairly uniformly between the clustering and 0.
%, meaning that there are some mutations that have bad locality.
Additionally there is a clustering at exactly 0.
%, indicating a discontinuity in the fitness metric.
%, which is most likely caused by the ``nothing'' metric that sets the score to 0 if there are no branches in the plant.
...
A Mann–Whitney U test on the sample without zero scores suggests that the true location shift is still not zero (p < 0.05, U = 1318100).
...
% should make the branches have a tendency to bend towards one side.
...
%indicating that it understands that focusing on a fewer set of variables may lead to less ``nothing'' plants.
...
Depth 1 is mostly producing operators, but also an ooccasional variable.
%, thus being useful and producing the lateral branches seen on the plant.
...
% This is mixed-method research.
% Embedded design, with qualitative data as supplementary.
% Phenomenological study? Fits somewhat...
% Grounded theory study? Ehh...
...
The demographic of the participant is mainly 20--30 year old (65\%) males (78\%) with a bachelor or higher degree (76\%) in information and communication technologies (49\%).
...
There were only 1 participant that received the share link from another participant.
...
\begin{figure}
    ...
    \caption[Fitness score plus geometric mean and arithmetic mean of priority weights]{Normalized fitness score (f) plus geometric mean (gm) of priority weights, with standard error}
    \label{fig:weights}
\end{figure}
...
Kendall's tau is 0.55 (p < 0.01), indicating a positive correlation, though only at about 55\%.
...
From this it can be argued that they agree that 0.27 is the worst plant.
0.34, 0.40, 0.53 and 0.84 seems to be ranked as equally bad, just above 0.27.
0.78, 0.72, 0.59 and 0.49 seem to be slightly climbing in score, but their standard errors are big, making it difficult to say for sure.
0.91 is clearly in first place, while 0.97 and 0.65 are possibly on a shared second place.
The standard errors are smaller on the bad plants than on the good plants, indicating that it is more likely that humans agree on what looks bad compared to what looks good.
% Thus maybe plants above a certain threshold should be used?

Both 0.84 and 0.46 are interesting cases as they were moved 5 places down and up respectively, almost swapping places.
...
This inferences are also supported by Kendall's Tau and to some degree dRank, as seen in Table~\ref{tab:rankstats}.
Kendall's Tau indicates that both closeness and drop are almost not correlated, and there is a high probability that the correlation is by chance.
Branching also has a low correlation, but relatively higher than the other two.
...
This is supported by the plot of the \textit{removal} evaluation in Figure~\ref{fig:fitcmp-removal} and its rank correlation in Table~\ref{tab:rankstats}.
...
The Kendall's Tau is 0.79 (p < 0.01), which is an improvement of 0.24, while dRank indicates no difference, possibly because of a less steep slope increasing the amounts of acceptable rank swaps.
...
\begin{table}
    ...
    \hline
    natural+ & Looking natural is better than artificial & 8 \\
    \hline
    natural- & Looking more artificial is interesting & 1 \\
    \hline
    symmetry+ & Symmetry is interesting & 1 \\
    \hline
    symmetry- & Symmetry makes plants look bad & 1 \\
    \hline
    chaos & Chaotic plants look bad (look busy) & 5 \\
    \hline
    more leaves & More leaves is better & 5 \\
    \hline
    balanced leaves & Balanced amount of leaves is better & 2 \\
    \hline
    lush & Plants that are lush look good (bush-like), while leafless plants look bad (weed-like) & 2 \\
    \hline
    leaf pitch & Leaves pointing is better than down & 1 \\
    \hline
    level of detail & Higher level of detail is more appealing & 4 \\
    \hline
    even spread & Branches and leaves spread out evenly to accord for moving sun & 3 \\
    \hline
    environment & Plants that look like they are affected by the environment are more immersive & 3 \\
    \hline
    size & Small plants look worse (need higher level of detail) & 3 \\
    \hline
    curves & More and soft curves is better, sharp angles look bad & 3 \\
    \hline
    upwards & Growing upwards is better, horizontal or down is bad & 2 \\
    \hline
    healthy & Looking healthy = better & 2 \\
    \hline
    complexity & Complex branches are better (simple could work with many in a field) & 2 \\
    \hline
    drop & Branches or leaves growing inside the ``pyramid'' look bad & 2 \\
    \hline
    patterns & Recognizable patterns are better & 1 \\
    \hline
    obstructions & Real plants have obstructions & 1 \\
    \hline
    ...
    \hline
    visible branches & Many visible branches look bad in a chaotic plant (should be covered by leaves) & 1 \\
    \hline
    organic & Organic look and shape & 1 \\
    \hline
    balanced & Mass center close to trunk & 1 \\
    \hline
    uniform bend & All branches should be bent in a similar way & 1 \\
    \hline
    \end{tabularx}
    \caption{Improving or deteriorating factors}
...
\begin{table}
    ...
    \hline
    \end{tabular}
    \caption{Distinguishing factors}
