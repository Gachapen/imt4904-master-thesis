...
While the fitness score does not depend on the hardware the experiment runs on, the fitness calculation duration does.
...
\gls{GE} uses a fixed number of generations and population size, resulting into a fixed number of total individuals: $individuals = population + population * generations$, that the brute-force method also will generate.
...
% Write about AHP?
A pairwise comparison of a set of \gls{DGEL}-generated plants will be performed by human participants and the \gls{AHP} will be used to create a ranking of all of the plants~\cite{2008Saaty}.
By using \gls{AHP}, we will not only get a ranking of the plants, but also distances between the ranks.
...
Because the human rankings only has priority weights and not absolute scores, the DGEL fitness scores will be translated into weights by dividing each score by the sum of the scores such that they sum to 1.
The data fit the requirements of Kendall's Tau~\cite{2010Webber}: they are unweighted, conjoint and have a definite range.
Therefore Kendall's Tau will be used to test the ranking correlation.
...
But there are two problems with this: it does not consider the relative distances between the ranks~\cite{2010Webber}, and the different participant's rankings will have to be aggregated beforehand, meaning Kendall's Tau does not consider the variance between the different human rankings.
Therefore, dRank will be used as an additional statistic, as it considers the distances between ranks and that there are multiple rankings aggregated together~\cite{2010Webber,2009Carterette}.
